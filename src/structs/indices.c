...
int compara_dados_int(const void* esquerda, const void* direita) {
    ...
    return sub != 0 ? sub : ((*(const dados_int_t**)esquerda)->byteOffset - (*(const dados_int_t**)direita)->byteOffset > 0 ? 1 : -1);
}
...
int compara_dados_str(const void* esquerda, const void* direita) {
    ...
    return comp != 0 ? comp : ((*(const dados_str_t**)esquerda)->byteOffset - (*(const dados_str_t**)direita)->byteOffset > 0 ? 1 : -1);
}
