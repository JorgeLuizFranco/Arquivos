...
void realiza_consultas(char* nome_arq_bin, char* nome_campo, char* tipo_campo, char* nome_arq_idx,
    ...
    if (funcionalidade == 5 || funcionalidade == 7) seta_consistencia_bin(arq_bin, cabecalho, '0');
    ...
    if (cabecalho_indice != NULL && (funcionalidade == 5 || funcionalidade == 7))
        seta_consistencia_ind(arq_idx, cabecalho_indice, '0');

    // se índice não estiver definido, libero memória e aborto
    // a não ser que seja a funcionalidade 4. Nesse caso, finge que apenas não existe arquivo de
    // índices e faz apenas buscas sequenciais
    ...
    for (int i = 0; i < num_consultas; i++) {
        ...
        int regs_mostrados =
            0; // serve para a funcionalidade 4. indica o numero de registros que foram mostrados
        if (funcionalidade == 4) printf("Resposta para a busca %d\n", i + 1);
        ...
        if (campos == NULL) {
            libera_memo_consultas(1, arq_bin, cabecalho, arq_idx, cabecalho_indice, NULL, -1, dados,
                                  num_indices);
            return;
        }
        ...
        if (funcionalidade == 7) {
            ...
            if (campos_atualizar == NULL) {
                libera_memo_consultas(1, arq_bin, cabecalho, arq_idx, cabecalho_indice, campos, -1,
                                      dados, num_indices);
                return;
            }
        }
        ...
        if (funcionalidade == 7)
            flag_campo_atualiza =
                checa_campo_procurado(campos_atualizar, num_campos_atualizar, nome_campo);

        // se o campo do indice nao tiver entre os campos especificados pela query, fazer uma busca
        // linear no arquivo binario
        if (flag_campo_procurado == -1) {
            ...
            while (byteOffset < byteOffsetFinal) {
                ...
                if (crime_atual == NULL) {
                    libera_memo_consultas(1, arq_bin, cabecalho, arq_idx, cabecalho_indice, campos,
                                          num_campos, dados, num_indices);
                    return;
                }

                // se não for logicamente removido e satisfizer a query
                if (crime_atual->removido != '1' &&
                    satisfaz_query(crime_atual, campos, num_campos)) {
                    ...
                    if (funcionalidade == 4) {
                        ...
                        int ind_arquivo = indice_procura_registro(crime_atual, byteOffset, dados,
                                                                  num_indices, nome_campo, tipoVar);
                        remove_com_shift(&dados, tipoVar, &num_indices, ind_arquivo,
                                         cabecalho_indice);
                    } else if (funcionalidade == 7) {
                        if (atualizar(arq_bin, cabecalho, crime_atual, byteOffset, arq_idx,
                                      cabecalho_indice, campos_atualizar, num_campos_atualizar,
                                      flag_campo_atualiza, campos, num_campos, nome_campo, dados,
                                      &num_indices, tipoVar, NULL) == 0) {
                            // abortar em caso de erro
                            libera_crime(crime_atual);
                            libera_vetor_ate_pos((void**)campos_atualizar,
                                                 num_campos_atualizar - 1);
                            libera_memo_consultas(1, arq_bin, cabecalho, arq_idx, cabecalho_indice,
                                                  campos, num_campos, dados, num_indices);
                            return;
                        }
                    }
                }
            ...
            // se tiver, ver todos os caras com o indice que quero no arquivo de indices com busca
            // binaria percorrer esses caras, e pra cada um deles eu vou pra cada byteoffset do
            // arquivo que to lendo e vejo se obedece os outros requisitos

            int baixo;
            int cima;
            long long int byteOffset;
            // pega uma chave de busca genérica de acordo com o tipo (string ou int) do campo
            void* chaveBusca = tipoVar == 0 ? (void*)&(campos[flag_campo_procurado]->chaveBuscaInt)
                                            : (void*)(campos[flag_campo_procurado]->chaveBuscaStr);
            // faz uma busca binária no vetor de índices para achar "baixo" e "cima" tais que
            // índices entre baixo e cima tem a chave buscada
            busca_bin_campos(dados, num_indices, &baixo, &cima, chaveBusca, tipoVar, 1);

            // vai iterando por esses caras que têm a chave buscada e faz uma query em cada um deles
            // pra ver se realmente obedecem aos requisitos para isso, vou incrementando "baixo"
            while (baixo <= cima) {
                ...
                // se o crime atual não tiver sido removido e ele satisfizer a query, ver a
                // funcionalide que quero e executá-la
                if (crime_atual->removido != '1' &&
                    satisfaz_query(crime_atual, campos, num_campos)) {
                    if (funcionalidade == 4) {
                        mostra_crime_tela(crime_atual);
                        ...
                        // se o índice da posição "baixo" foi removido, a shiftação fez com que
                        // "baixo" agora aponte para o próximo registro. Mas só quero fazer isso na
                        // próxima iteração do loop então desconto isso
                        baixo--;
                        ...
                        if (atualizar(arq_bin, cabecalho, crime_atual, byteOffset, arq_idx,
                                      cabecalho_indice, campos_atualizar, num_campos_atualizar,
                                      flag_campo_atualiza, campos, num_campos, nome_campo, dados,
                                      &num_indices, tipoVar, &flag_nova_pos) == 0) {
                            libera_crime(crime_atual);
                            libera_vetor_ate_pos((void**)campos_atualizar,
                                                 num_campos_atualizar - 1);
                            libera_memo_consultas(1, arq_bin, cabecalho, arq_idx, cabecalho_indice,
                        ...
                        if (flag_nova_pos <= baixo) {
                            ...
                            // se menor que baixo: não faço nada. Porque agora que ele ajustou sua
                            // posição no vetor para um índice inferior, o cara que tava em baixo-1
                            // agora está em baixo. Mas isso não importa, pois basta esperar a
                            // próxima iteração para baixo incrementar e eu ir pro próximo

                            // se igual a -1: existem apenas as seguintes possibilidades do que pode
                            // ter ocorrido
                            // - foi feita uma inserção (mas é impossível isso ocorrer, pois
                            // inserções só ocorrem quando o campo vai de NULL para algo não nulo.
                            // Mas era impossível ele ser inicialmente NULL, pois o registro atual
                            // foi obtido com auxílio do arquivo de índices)
                            // - o campo referente ao índice não sofreu atualização e o seu
                            // byteoffset não mudou (ignoro)
                            ...
                            // entao agora "baixo" aponta precocemente para o proximo cara por conta
                            // da shiftação , entao conserto isso decrementando
                            baixo--;
                            if (flag_nova_pos <= cima) {
                                // mas se aumentou e continua <= cima quer dizer que a chave nao
                                // mudou ou seja, ele foi inserido no final do arquivo binario de
                                // registros e esta com o maior byte offset de todos nesse caso, ele
                                // só pode estar na posicao "cima" entao eu nem preciso mais
                                // verificar essa posicao por isso, eu tiro ela do intervalo
                                cima--;
                            } else {
                                // por outro lado, se aumentou e ficou > cima, quer dizer que o cara
                                // que estava em "cima" foi shiftado uma vez para a esquerda para
                                // adequar o intervalo a isso, faz-se
                                cima--;
                            }
                        }
    ...
    libera_memo_consultas(0, arq_bin, cabecalho, arq_idx, cabecalho_indice, NULL, -1, dados,
                          num_indices);
