 ...
 */
void busca_bin_campos(void** ind_int, int num_regs, int* baixo_reg, int* alto_reg, void* chaveBusca,
                      int tipo_var, int flag_dinamica) {
    // a busca binária é feita com base na função compara_chave_busca de nome auto-explicativo, que
    // lida com o caso em que a chave é int ou string

    // quero que *baixo aponte pro menor para e *alto aponte para o maior cara que satisfaçam a
    // busca por enquanto, seto como -1 e -2 seus valores, como indicação de que não existe a chave
    // buscada
    *baixo_reg = -1;
    *alto_reg = -2;
    ...
    // se resultado = -1, quer dizer que nao tem nenhum índice >= à chave de busca. Portanto ela não
    // está presente se algum resultado for encontrado, mas esse resultado for uma chave de busca
    // diferente da que quero, então quer dizer que eu apenas consegui encontrar uma chave de busca
    // maior nesse vetor em ambos os casos, paro aqui e continuo com -1 e -2 como valores finais
    if (resultado == -1 ||
        compara_chave_busca(ind_int[resultado], chaveBusca, 0, tipo_var, flag_dinamica) == 0)
        return;
    *baixo_reg = resultado;

    // agora que sei que a chave de busca está presente, procuro pelo menor índice que é
    // estritamente maior que a minha chave de busca e guardo em "resultado" faço isso usando uma
    // versão da busca binária que implementa o clássico algoritmo de upper bound
    ...
    // então dito isso, a única possibilidade é que a partir do índice *baixo_reg, todos os
    // registros tem essa mesma chave de busca

    // se resultado for diferente de -1, quer dizer que realmente esse menor indice estritamente
    // maior existe. Então pego ele menos 1, porque alguém menor que ele tem que ser menor ou igual
    // a chave de busca em particular, será obviamente igual
...
int indice_procura_registro(crime_t* crime_atual, long long int offset_atual, void** dados,
    ...
    void* dado_crime =
        pega_dado_generico(crime_atual, nome_campo, -1,
                           tipoVar); // so serve pra pegar chave busca a partir do nome do campo
    if (dado_crime == NULL) return -2;

    // se o campo "nome_campo" do crime for NULO, esse crime com certeza não está no arquivo de
    // índice se não for nulo (==0), pode estar, então com isso encontra-se baixo_reg e alto_reg,
    // que são os índices tais que entre eles todos tem a chave buscada
    if (checa_dado_nulo(dado_crime, tipoVar) == 0)
        busca_bin_campos(dados, num_dados, &baixo_reg, &alto_reg,
                         pega_chave_generico(dado_crime, tipoVar), tipoVar, 0);
    else
