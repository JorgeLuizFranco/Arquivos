 ...
 * @param campos_atualizar vetor de struct que guarda um campo da struct e o valor para o qual será atualizado crime_atual
 * @param num_campos_atualizar tamanho desse vetor
 * @param flag_campo_atualiza indica se o arquivo de índices foi feito com base em um campo que terá seu valor mudado nessa chamada
 * de função
 * @param campos campos (vetor) da query. Irrelevante para a função em si. Só é passado como parâmetro porque tenho que dar free em caso de erro
 ...
 * @param flag_nova_pos se não for NULL, será setada pela função com a nova posição do registro de índices
 * no vetor de índices (dados). É basicamente um "segundo retorno"
...
int atualizar(FILE* arq_bin, cabecalho_t* cabecalho, crime_t* crime_atual, long long int byteOffset, FILE* arq_idx, cabecalho_indice_t* cabecalho_indice, 
    ...
    // se o tamanho do crime atualizado for menor ou igual ao tamanho que o crime_atual ocupa no arquivo
    // então é possível fazer a sobrescrição
    ...
    if (flag_campo_atualiza != -1 || byteOffset_final != byteOffset) {
        ...
        if (checa_dado_nulo(dado_crime_antes, tipoVar) && checa_dado_nulo(dado_crime, tipoVar) == 0) {
            ...
            if (flag_nova_pos != NULL) *flag_nova_pos = nova_pos; // seta a nova posição do registro de índice no vetor de índices
        }
